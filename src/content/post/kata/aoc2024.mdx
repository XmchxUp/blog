...
首先是遍历所有点，从当前点出发，使用BFS遍历所有相关的region中的点。

即可获得所有region

partI: 边长计算：对每个region中的点，可以先计算都有4个边，但是如果有一边与region中的点相连，则减少一边。
...
边长计算：对每个region中的点p，都有4个corner坐标，遍历每个corner坐标，判断四周的点是否是属于region中的，

如果是，则有四种情况，只有1/2/3/4个点属于region。

1/3 只有一个corner，即一条边

4没有corner

2 分为只有对角点有两个conrner，相邻点没有conrner
