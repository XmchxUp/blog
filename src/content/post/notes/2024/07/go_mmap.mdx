---
category: "Technology"
date: "2024-07-11 22:40"
title: "Golang怎么通过mmap执行汇编代码"
tags: ["Practice", "CS", "Go"]
---
...
按理说已经是根据上面的思路是可以的，比如我试了下 x86_64 格式的汇编指定(由 FASM 生成)是不行的(see [Code](https://github.com/Ysoding/jitbf/blob/main/runbin/main.go))
...
由于 Golang 中的机制限制，同样的逻辑用 rust 或 C 都是 Ok 的 [Rust Code](https://github.com/Ysoding/jitbf/blob/main/runbin/rb/src/main.rs)（l 是否逻辑使用 C,然后在 go 中用 cgo 调用 C）。
